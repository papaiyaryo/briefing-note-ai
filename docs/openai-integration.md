...
1. `OCR_PROVIDER` / `STRUCTURE_PROVIDER` が `openai` または `dummy` の場合は明示指定を優先する。
...
## コスト方針 TODO（詳細は Issue #36）

- 1 メモあたり OCR と構造化で最大 2 回の OpenAI API 呼び出しが発生する前提で整理する。
- 画像サイズ上限、出力トークン上限、タイムアウトを決める。
- 低コスト既定モデルと高精度オプションの切替方針を、実装時点の公式 docs と料金を確認して決める。
- API キー未設定時はダミーにフォールバックし、CI / テストでは課金が発生しないようにする。

## ログ方針 TODO（詳細は Issue #36）

- エラーコードと最小限のメタデータだけを記録するヘルパーを用意する。
- provider error の内部詳細はサーバーログにも必要最小限とし、レスポンスには安全な日本語メッセージだけを返す。
- 画像・OCR 全文・Markdown・API キーがログに混入しないことをレビュー項目にする。
- レート制限、タイムアウト、検証失敗を安全に分類する。
